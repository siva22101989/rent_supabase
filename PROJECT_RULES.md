...
- **Synchronized Updates**: Any change to the database schema (SQL migrations) MUST be immediately followed by corresponding updates to the frontend codebase.
- **Scope of Update**: This includes updating TypeScript interfaces/types, adjusting database queries (Supabase/Postgres), modifying Server Actions, and updating UI components to reflect the schema change.
- **Validation**: Ensure that Zod schemas or input validation logic matches the new database constraints.
- **Soft Deletes**: If soft delete (`deleted_at`) is added, ensure queries filter these records and actions perform updates instead of hard deletes.
